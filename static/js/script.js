			...
			function send_data(item) {
				...
				function success_function(checkbox_name, curr_response) {
					...
					$.each(curr_response, function (k, v) {
						if(checkbox_name == 'amazon_checkbox'){
							create_amazon_product_details_div(k, v, checkbox_name);
						}
						else if(checkbox_name == 'ebay_checkbox'){
							create_ebay_product_details_div(k, v, checkbox_name);
						}
				...
				function create_amazon_product_details_div(item_number, product_data, curr_checkbox_name) {
					...
					// Product Price
					let txt_sale_price = "<div class='results_products_item_subtitle'>Sale Price: </div>"
					...
					$(parent).append(
						$('<div>').prop({
							innerHTML: txt_sale_price + product_data['item_price'],
							className: 'results_products_details'
						})
					);

					// Old Price
					let txt_old_price = "<div class='results_products_item_subtitle'>Regular Price: </div>"
					if (product_data['old_price'] != "Unavailable") {
						txt_old_price += "$"
					}

					$(parent).append(
						$('<div>').prop({
							innerHTML: txt_old_price + product_data['old_price'],
							className: 'results_products_details'
						})
					);
					...
					$(parent).append(
						$('<a>').prop({
							innerHTML: "<div class='results_products_item_subtitle'>Percentage Off: </div>" + product_data['percentoff']+"%",
							className: 'results_products_details'
						})
					);
				...
				function create_ebay_product_details_div(item_number, product_data, curr_checkbox_name) {
					item_number = parseInt(item_number, 10) + 1;
					let parent = '#' + curr_checkbox_name + '_result';
					...
					// Product Price
					let txt_sale_price = "<div class='results_products_item_subtitle'>Item Price: </div>"
